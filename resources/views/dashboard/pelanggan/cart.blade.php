...
@section('content')
    ...
    <div class="cart-container">
        ...
        @else
                    ...
                    <div class="cart-item" style="flex-direction: column; align-items: stretch;">
                        <div style="display: flex; gap: 20px; align-items: center; width: 100%;">
                            @if ($item->menu->image && (strpos($item->menu->image, '/') !== false || strpos($item->menu->image, '.') !== false))
                                <img src="{{ asset('storage/' . $item->menu->image) }}" class="cart-item-img"
                                    alt="{{ $item->menu->name }}">
                            @else
                            ...
                            <div class="cart-item-info">
                                ...
                                <form action="{{ route('dashboard.pelanggan.cart.update', $item->id) }}" method="POST"
                                    id="form-qty-{{ $item->id }}">
                                    @csrf
                                    @method('PUT')
                                    <div class="qty-controls">
                                        <button type="button" class="btn-qty" onclick="updateQty({{ $item->id }}, -1)">-</button>
                                        <input type="number" name="quantity" id="qty-{{ $item->id }}" value="{{ $item->quantity }}"
                        ...
                        @if($index === 0)
                            <div
                                style="border-top: 1px dashed #f0e6d2; padding-top: 20px; margin-top: 10px; display: grid; grid-template-columns: 1fr 2fr; gap: 20px;">
                                <div class="form-group" style="margin-bottom: 0; grid-column: span 2;">
                                    <label for="notes"
                                        style="display: block; font-weight: 700; color: var(--brand-dark); margin-bottom: 8px; font-size: 0.9rem;">Catatan
                                        Pesanan</label>
                                    <textarea name="notes" id="notes" rows="1" placeholder="Contoh: Es teh gulanya dikit aja"
                                        style="width: 100%; padding: 12px 15px; border: 2px solid #f0e6d2; border-radius: 12px; background: var(--brand-cream); font-family: inherit; resize: vertical;">{{ $order->notes }}</textarea>
                                </div>
                            </div>
                        @endif
                    </div>
            ...
            <!-- Hidden table number -->
            <input type="hidden" id="table_number" value="{{ $order->table_number }}">

            <!-- Checkout Section -->
            <div class="checkout-bar">
                <form action="{{ route('dashboard.pelanggan.cart.checkout') }}" method="POST" id="checkout-form"
                    class="checkout-form">
                    @csrf
                    <div class="checkout-summary" style="margin-top: 0; padding-top: 0; border-top: none;">
                        ...
                        <button type="button" id="pay-button" class="btn-checkout">Bayar Sekarang</button>
                    </div>
                </form>
            </div>
        @endif
        ...
        document.getElementById('pay-button').onclick = function () {
            ...
            fetch("{{ route('dashboard.pelanggan.cart.checkout') }}", {
                ...
                .catch(error => {
                    ...
                    let errorMsg = "Terjadi kesalahan sistem saat memproses checkout.";
                    if (error.error) {
