...
@section('content')
    <style>
        ...
        .table-card.occupied .table-info {
            color: #EF4444;
            font-weight: 600;
        }
    ...
    <div style="max-width: 1100px;">
        ...
        <div class="dashboard-header"
            ...
            <a href="{{ route('dashboard.waiter.orders') }}" class="btn-primary"
                style="padding: 12px 24px; font-size: 1rem; border-radius: 50px; display: flex; align-items: center; gap: 8px;">
                📝 Daftar Pesanan
            </a>
        </div>
        ...
        <div class="table-grid">
            @foreach($tables as $table)
                <div class="table-card {{ $table->status }}"
                    onclick="showTableDetail('{{ $table->number }}', '{{ $table->status }}', '{{ $table->guest_name ?? '' }}', '{{ $table->reserved_since ? \Carbon\Carbon::parse($table->reserved_since)->format('H:i') : '' }}')">
    ...
    <div id="tableModal"
        ...
        <div
            style="background:white; border-radius:20px; padding:35px; max-width:400px; width:90%; position:relative; box-shadow: 0 20px 60px rgba(0,0,0,0.2);">
            <button onclick="closeModal()"
                style="position:absolute; top:15px; right:18px; background:none; border:none; font-size:1.4rem; cursor:pointer; color:#999;">✕</button>
